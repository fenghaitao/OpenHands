...
async def auto_setup_github_copilot_on_startup(user_id: Optional[str] = None) -> bool:
    ...
    try:
        ...
        mode = detect_github_copilot_mode()
        if not mode:
        ...
        success = await setup_github_copilot_settings(config.file_store_path, mode)
...
def setup_github_copilot_on_startup_sync(user_id: Optional[str] = None) -> bool:
    ...
    try:
        ...
        if loop.is_running():
            ...
            task = asyncio.create_task(auto_setup_github_copilot_on_startup(user_id))
            logger.debug("Scheduled GitHub Copilot auto-setup task")
            return False  # Can't wait for result in running loop
        else:
            return loop.run_until_complete(auto_setup_github_copilot_on_startup(user_id))
    except RuntimeError:
        # No event loop, create a new one
        return asyncio.run(auto_setup_github_copilot_on_startup(user_id))